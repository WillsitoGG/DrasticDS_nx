...
#define SO_MAX_JIT_RANGES 4

typedef struct {
  size_t offset;
  size_t size;
  void *rw_alias;
  void *rw_memrv; // VirtmemReservation *
  uint32_t handle;
  int mapped;
} SoJitRange;
...
typedef struct so_module {
  ...
  /* Android permits a single RWX address for JIT caches. Horizon enforces
   * W^X, so selected subranges are backed by a CodeMemory object with the
   * original module address RX and a separate host-only RW alias. */
  SoJitRange jit_ranges[SO_MAX_JIT_RANGES];
  int jit_range_count;
