 ...
 *
 * The Android core imports dlopen/dlsym/dlclose for optional host libraries
 * (notably libEGL.so) and dlsym()s entry points (e.g. eglGetProcAddress), and
 * occasionally requests optional symbols via dlsym(RTLD_DEFAULT, name). We don't
...
void *dlopen_fake(const char *filename, int flags) {
  ...
  dl_err_set = 0;
  // any name (libEGL.so, a plugin, self) -> the same pseudo handle; dlsym
...
void *dlsym_fake(void *handle, const char *symbol) {
  ...
  // 1) host import table (libc/EGL/OpenSL ES/...); eglGetProcAddress is hooked
