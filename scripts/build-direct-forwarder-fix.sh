...
EXPECTED_TUNED_NRO_SHA256="91ebec3eab02001609d12fe267fdb190fdffa1ad690ccb48ba63a03e3d321542"
...
echo "==> Installing Switch build dependencies"
...
echo "==> Downloading exact official DrasticDS_nx 1.0.9 release asset"
python3 - <<'PY'
import hashlib
import json
import pathlib
...
root=pathlib.Path('/work/drastic-willsito-direct-forwarder/official')
...
pathlib.Path('/work/drastic-willsito-direct-forwarder/DrasticDS_official.nro').write_bytes(data)
pathlib.Path('/work/drastic-willsito-direct-forwarder/OFFICIAL_RELEASE.txt').write_text(
...
if [[ "$ACTUAL_TUNED_SHA256" != "$EXPECTED_TUNED_NRO_SHA256" ]]; then
  echo "Tuned NRO SHA mismatch." >&2
  echo "Actual:   $ACTUAL_TUNED_SHA256" >&2
  echo "Expected: $EXPECTED_TUNED_NRO_SHA256" >&2
  echo "Use the validated reference environment: devkitpro/devkita64:20260219" >&2
  exit 1
fi
...
Tuned NRO SHA-256: $ACTUAL_TUNED_SHA256
...
echo "PASS: exact validated DrasticDS_nx Direct Forwarder Fix reproduced"
