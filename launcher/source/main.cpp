...
int main(int argc, char **argv){
  ...
  // Standard NSP forwarders place the selected ROM directly in argv[1].
  // Detect that mode before storage/library startup so a direct launch never
  // needs to render the launcher grid or its USB-connection waiting screen.
  ...
  const bool startupHasUsbSource=(hasUsbSource&&!silentDirectForwarder)||directForwarderUsesUsb;
  ...
  std::thread storageInitWorker([&,startupHasUsbSource,startupSmbShares]{
    ...
    // A positional SD/USB forwarder does not need the normal launcher library
    // or SMB auto-mount path. Avoid unrelated startup work before chainloading.
    if(!silentDirectForwarder) for(const auto &share:startupSmbShares){
  ...
  auto prepareDirectForwarderGame=[&](const std::string &directPath) -> bool {
    ...
    game.hasCfg=regularFileExists(std::string(GAMECFG_DIR)+"/"+game.key+".ini")||
                regularFileExists(std::string(GAMECFG_DIR)+"/"+game.pathKey+".ini");
    // Basename uniqueness is deliberately not guessed without a library scan.
    // Stable-key and path-key profiles remain fully supported.
  ...
  auto findForwarderGame=[&]() -> Game* {
    return findGameByKey(forwarderKey);
  };
  if(forwarderDirectPath)
    forwarderMatched=prepareDirectForwarderGame(forwarderKey);
  ...
  while(running&&beginUiFrame()){
    ...
    if(forwarderPending){
      ...
      if(!running) break;
      // Built-in -g shortcuts keep the informative launcher wait screen.
      // Raw positional forwarders stay completely silent: no grid and no
      // "connecting USB storage" frame is ever presented before the game.
